...
gcloud run deploy enrichment-trigger-v4 \
  ...
  --min-instances=0 \
  --max-instances=2 \
  --set-env-vars="PROJECT_ID=profitscout-fida8,DATASET=profit_scout,GCS_BUCKET=profit-scout-data,TRACE_LOGGING_ENABLED=false,MIN_ENRICHMENT_SCORE=1" \
