...
Shared content lib vendored at deploy time into `x-poster` + `blog-generator` (and the future `content-drafter` extension). Contains: `voice_rules` (DO/DO NOT, retired aliases, banned phrases, canonical disclaimer strings), `compliance` (`score_against_rubric`, `canonicalize_draft_text` for X posts, `canonicalize_blog_disclaimer` for blog markdown), `brand` (hex codes, fonts), `firestore_helpers` (idempotency keys, x_posts log writer, todays_pick reader), `tweepy_helper` (X posting + DRY_RUN), `mcp_client` (used by the agent). Single source of truth for any voice/compliance change — services pick it up at next `bash deploy.sh`.
