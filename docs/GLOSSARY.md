...
| `signal-judge` | The V5.4 ranker (renamed from `signal-ranker` 2026-06-04). One memory-aware `gemini-3.1-pro-preview` judge (`judge_v6`) scores every gated candidate and picks one (+ runner-up + confidence) in a single call, reading enriched candidates + daily report + 14d ledger + closed-trade case-memory. Writes `signal_ranker_runs` (table name unchanged). | The one high-stakes daily decision. Collapsed from a Scorer+Picker pair 2026-06-04. Fail-closed — no fallback ranker. |
