...
| `enrichment-trigger` | Filters scanner output to signals with `overnight_score >= 1 AND directional UOA > $500k` (all directions), then edge-ranks + grounds only the **top-50 BULLISH** names (`thinking_budget=0`). Adds features: premium flags, technicals, V/OI ratio, moneyness %, VIX3M, `mom_60`. | Turns raw noise into tradeable candidates. The 2026-06-05 raise of the score floor to 4 never deployed (the `deploy.sh` env pin held it at 1); the owner accepted the floor of 1 on 2026-08-20 — cosmetic, the UOA bar + top-50 cap do the filtering (`docs/DECISIONS/2026-08-20-score-floor-accepted-print-floor-25-shipped.md`). The spread gate was RETIRED 2026-06-05 (this Polygon plan serves no NBBO quotes, so `recommended_spread_pct` is permanently NULL). |
| `signal-notifier` | Applies the two SAFETY rails (VIX ≤ VIX3M regime gate, no earnings during hold window), hard-gates to BULLISH, edge-ranks + caps the pool, re-fetches **live OI** and drops dead contracts (`OI_FLOOR`), calls `signal-judge` for the bracket-tournament pick, emails you the **top 1** at **~09:52 ET**. Also writes `cohort_stats/current` (public-stats panel) and the canonical `todays_pick/{scan_date}` doc. | Your inbox is the signal. One pick per day or nothing. The per-candidate selection gates were all REMOVED in V6 (2026-06-04); cron moved 09:00 → 07:30 ET (2026-05-06), then to ~09:45 ET (2026-06-25) with the live-OI floor, then to 09:52 ET (2026-07-28). |
| `signal-judge` | The V6 ranker (renamed from `signal-ranker` 2026-06-04). A randomized bracket **tournament** (`tournament_v1` family, currently `tournament_v1_3` — see `signal-judge/app/agent.py`; `gemini-3.1-pro-preview`) over the capped tournament pool — 3 brackets × (batches ≤10 → top-2 advance) → consensus pick (+ runner-up + confidence). Simple prompt + daily report + per-contract JSON; no memory/rubric/weights. Writes `signal_ranker_runs` (table name unchanged). | The one high-stakes daily decision. Evolved Scorer+Picker → judge_v6 → tournament across 2026-06-04. Fail-closed — no fallback. |
...
| `blog-generator` | ADK multi-agent weekly blog publisher. Mon 05:00 ET cron → `POST /generate` → `blog_posts/{slug}` in Firestore → webapp renders. 13-row schedule per the 90-day plan. Also carries the built email endpoints (`/draft_email` + blast to the Firestore `users` collection on approval) — drafts only, never auto-sends. | Long-form SEO + cornerstone content. The blog is the hub; X / Reddit / email are spokes. |
...
| `forward_paper_ledger` | Paper P&L for the daily pick (one row per `scan_date`). Tagged by `policy_version` — current rows are `V7_1_TILTED_GIGO`. The live cohort start is `LIVE_COHORT_START_DATE` in `signal-notifier/main.py` (2026-08-21 as of the 2026-08-20 reset); cohort resets keep the rows and move the date filter. | `forward-paper-trader` | Your live paper scoreboard. Compare cohort EV here to your real P&L to see if discretion adds value. |
...
| `blog_schedule/current` | 13-row 90-day plan. Each row: `slug`, `week_num`, `title_candidate`, `persona`, `keywords`, `cta` (webapp_visit / starter_trial / pro_trial), `type`, `cross_channel`, `status` (pending / published). | `scripts/seed_schedule.py` (run with `PROJECT_ID=profitscout-fida8`) | blog-generator planner + cross-channel coordination. |
...
| `users` | Webapp signups. All have `email`. Other fields: displayName, isAnonymous, isSubscribed, plan, uid, daysActive, usageCount, createdAt, stripeCustomerId. | webapp signup flow | Email blast audience for blog-generator `/draft_email`. |
...
| Phase 2 backlog | Sweep/block detection, aggressor side, GEX, regime-conditional sizing — deferred. Each ships as its own `docs/DECISIONS/` note with owner approval and evidence. |
...
| `gammarips-review` | Read-only auditor. Optional and owner-invoked (owner call 2026-08-19). Checks lookahead, leakage, unsafe execution. |
| `gammarips-seo` | Read-only SEO / organic-traffic analyst. Pulls Search Console + GA4 data via `scripts/seo/`. Recommends, never mutates. |
...
| `docs/wiki/_index/REGISTRY.md` | You want the one-claim answer to "what runs and why" or any policy lineage. |
