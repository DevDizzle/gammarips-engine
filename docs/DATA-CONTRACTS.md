...
- **Full-life surface (`life_*`, added 2026-07-08)** — the opp surface extended to the contract's WHOLE life: `life_peak_return` / `life_trough_return` = max/min excursion from the SAME 10:00 ET fill (`opp_entry_price` anchor) through **expiration** (minute-resolution opp excursion for days 1–3 ∪ Polygon daily bars from entry_day+1; the entry day's daily bar is excluded — it contains pre-entry prints); `life_expiry_return` = hold-to-settlement mark, **intrinsic at expiration** from `underlying_daily_bars` (not the option's last, often stale, print). NO exit rule; **NOT a tradeable label, NOT a feature**. `life_status` ∈ {OK, PARTIAL_NO_EXPIRY, PARTIAL_NO_PATH, NO_ENTRY, ERROR}; NULL = not yet expired/labeled (the work queue). Written only once `recommended_expiration < today ET` (final session complete) by the `/label_life_surface` cron (17:10 ET) + one-shot `backfill_life_surface.py`; mechanics stamped in `life_sim_version='LIFE_TO_EXPIRY_V1'`. Powers the public Track Record distributions via the `/pool_outcomes` aggregates. See `docs/DECISIONS/2026-07-08-scorecard-life-distribution.md`.
