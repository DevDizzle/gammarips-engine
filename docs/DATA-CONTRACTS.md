...
Active forward paper-trading ledger. Written by `forward-paper-trader/main.py:run_forward_paper_trading` via delete-then-load JSON-L. One row per `scan_date` (one-pick-per-day ledger; the trader simulates ONLY the ticker named in `todays_pick/{scan_date}`). **Mechanics (V7.1 GIGO):** 10:00 ET entry, −30% hard stop, +40% target, same-day hold (`HOLD_DAYS=1`), flat 15:45 ET exit; **no trail** (`USE_TRAIL=False`). On ambiguous bars **TIMEOUT(15:45) > STOP > TARGET** (conservative). Rows are tagged `policy_version = 'V7_1_TILTED_GIGO'` (cohort start: the live value is `LIVE_COHORT_START_DATE` in `signal-notifier/main.py`, 2026-08-21 as of the 2026-08-20 reset). **Truncation history:** the ledger was physically truncated twice, at the V6 launch (2026-06-04, wiped the `V5_4_AGENT_RANKER` rows) and at the V7.1 relabel (2026-06-22). The five later cohort resets (2026-06-25, 2026-07-28, 2026-08-07, 2026-08-12, 2026-08-20) kept all rows and moved `LIVE_COHORT_START_DATE` instead; the excluded rows stay in the table. Do NOT mix cohorts across `policy_version`, and do not mix across cohort start dates either. Populated by Cloud Scheduler `forward-paper-trader-trigger` at 16:30 ET Mon-Fri. The cron resolves `scan_date` such that `exit_day = today` (same-day hold: `exit_day = entry_day`, via `get_canonical_scan_date`).
...
- `trail_trigger_price`, `peak_premium`, `trail_activated`, `trail_stop_at_exit` — all inert under V7.1 (`USE_TRAIL=False`) but written on every row for schema stability
...
Counterfactual bracket-replay option-PnL labels over the **full** enriched BULLISH pool (~50 rows/day), so a leakage-safe label set accrues ~50x faster than the 1-pick/day ledger. Written by the `/label_enriched_pool` cron via a mechanical replay of `forward-paper-trader/main.py:_simulate_contract` (no LLM). **HARD ISOLATION: research-only** — per-contract rows are never read by the webapp / blog; the ONE sanctioned public egress is `win-tracker /pool_outcomes`, which publishes **pool-level aggregates only** (quantiles + fixed-edge histogram buckets) to Firestore `pool_outcomes/current` for the Track Record page (owner call 2026-07-03; life distributions added 2026-07-08). Partitioned by `entry_day` (DAY), clustered by `ticker`. Schema source of truth: `scripts/ledger_and_tracking/create_enriched_option_outcomes.py`.
...
- **Full-life surface (`life_*`, added 2026-07-08)** — the opp surface extended to the contract's WHOLE life: `life_peak_return` / `life_trough_return` = max/min excursion from the SAME 10:00 ET fill (`opp_entry_price` anchor) through **expiration** (minute-resolution opp excursion for days 1–3 ∪ Polygon daily bars from entry_day+1; the entry day's daily bar is excluded — it contains pre-entry prints); `life_expiry_return` = hold-to-settlement mark, **intrinsic at expiration** from `underlying_daily_bars` (not the option's last, often stale, print). NO exit rule; **NOT a tradeable label, NOT a feature**. `life_status` ∈ {OK, PARTIAL_NO_EXPIRY, PARTIAL_SPLIT_ADJUSTED, PARTIAL_NO_PATH, NO_ENTRY, ERROR}. **Work queue (changed 2026-08-07) = NULL ∪ `PARTIAL_NO_EXPIRY`-whose-settlement-bar-now-exists**, not NULL alone: a bars-cache gap used to strand rows in `PARTIAL_NO_EXPIRY` permanently (726 of them, see `docs/DECISIONS/2026-08-07-freshness-canary-and-bars-loader.md`), so they are re-queued once the bar they need lands. `PARTIAL_SPLIT_ADJUSTED` is TERMINAL: the underlying split after selection, so the frozen `recommended_strike` and the settlement close are in different units and no intrinsic can be computed honestly — excluded from the public `OK` cohort rather than published as a fabricated −100%. Written only once `recommended_expiration < today ET` (final session complete) by the `/label_life_surface` cron (17:10 ET) + one-shot `backfill_life_surface.py`; mechanics stamped in `life_sim_version='LIFE_TO_EXPIRY_V1'`. Powers the public Track Record distributions via the `/pool_outcomes` aggregates. See `docs/DECISIONS/2026-07-08-scorecard-life-distribution.md`.

### Column classification (the leakage boundary)

Every column belongs to exactly one group. The classification is written into the BQ **column descriptions** (machine-readable) by `scripts/ledger_and_tracking/tag_enriched_column_descriptions.py`, prefixed `[feature|label|opportunity|regime_telemetry|identity | as-of <boundary>]`. Adopt the prefix convention going forward: `label_*` = label-semantics tag, `oc_*` = entry-close regime telemetry (realized after the trade), `opp_*` = opportunity-surface excursion, `life_*` = full-life (surfaced→expiration) surface.
...
- **OPPORTUNITY** (`opp_*`, `life_*`): exit-free excursion surfaces (3-day window and full life) — not a label, not a feature.
...
- **`enriched_features_v1`** — VIEW over `enriched_option_outcomes` exposing ONLY the FEATURE + IDENTITY + cohort-meta allowlist above. Created by `scripts/ledger_and_tracking/create_enriched_features_view.py` (dry-run default, `--execute` to apply).
...
## Shadow ledger — `profitscout-fida8.profit_scout.paper_shadow_topscore` (added 2026-06-08)

Research-only A/B ledger: tournament pick vs the deterministic top-`overnight_score` pick. Written by `forward-paper-trader` in the same exit-cron invocation as the live ledger row, on HAS_PICK days only. Two rows per day (arm `TOURNAMENT` + arm `TOP_SCORE`), both simulated with the identical `_simulate_contract` mechanics. Best-effort: the writer never raises into the live trade path. **Hard isolation:** it writes ONLY to this table (plus the companion `paper_shadow_intraday`) and is NEVER surfaced to the Scorecard, the website, or any public aggregate. See `docs/DECISIONS/2026-06-08-topscore-shadow-tracker.md`.

## Settlement-bars cache — `profitscout-fida8.profit_scout.underlying_daily_bars`

Daily underlying OHLC cache. It is the settlement-close source for the life surface's `life_expiry_return` (intrinsic at expiration), so the `/label_life_surface` labeler depends on it. Scope: the ~860 substrate tickers (`enriched_option_outcomes` ∪ `overnight_signals_enriched`), not the full market. Loaded by `forward-paper-trader` `POST /load_underlying_bars` (weekdays 17:05 ET) plus the one-shot `scripts/ledger_and_tracking/load_underlying_daily_bars.py`. Writes are idempotent per-date DELETE-then-LOAD and refuse an empty row list, so a Polygon outage is a no-op retried the next day. The dbt freshness canary watches this table. See `docs/DECISIONS/2026-08-07-freshness-canary-and-bars-loader.md`.
...
Per-trade publish of the closed live cohort (current: V7.1) for the public webapp scorecard table (`/scorecard`). Written by `signal-notifier/main.py:compute_and_write_ledger_trades` alongside `cohort_stats/current`, on the same daily cron and the `/refresh_stats` endpoint. **Uses the identical cohort filter and fixed-dollar sizing as `cohort_stats/current`** (`DATE(entry_timestamp) >= LIVE_COHORT_START_DATE` (the live value is in `signal-notifier/main.py`, 2026-08-21 as of the 2026-08-20 reset) AND `policy_version = 'V7_1_TILTED_GIGO'` AND `realized_return_pct IS NOT NULL` AND `entry_price > 0`; `n_contracts = GREATEST(1, ROUND(POSITION_SIZE_USD/(entry_price*100)))`), so the table rows and the aggregate tiles can never disagree. Idempotent upsert (`merge=True`) keyed by `{scan_date}_{ticker}`; non-gating, display-only. Read-only consumer; never feeds any execution gate.
