...
# Config block for V4 — No Gate, Spread Only
# See docs/DECISIONS/2026-04-12-v4-fresh-start.md
#
# V4 philosophy: let everything through, collect execution data, discover what
# wins after 30 days via tree-based feature importance. The ONLY execution
# filter is spread <= 10%. Premium flags are still computed upstream and stored
# for post-hoc discovery — they just don't gate trades.
MAX_SPREAD_PCT = 0.10
HOLD_DAYS = 2
POLICY_VERSION = "V4_NO_GATE_SPREAD_ONLY"
POLICY_GATE = "SPREAD_LE_10PCT_ONLY"
LEDGER_TABLE = f"{PROJECT_ID}.profit_scout.forward_paper_ledger_v4_hold2"
...
def run_forward_paper_trading(target_date: date = None):
    """V4 forward paper trading — no gates, spread <= 10% only.

    Config:
      - 2-day hold window
      - Spread <= 10% is the only execution filter
      - Writes to forward_paper_ledger_v4_hold2
    ...
    # V4: no trader-side gates. Spread filtering happens upstream in enrichment.
    # Everything in the enriched table is tradeable. See docs/DECISIONS/2026-04-12-v4-fresh-start.md
    query = f"""
    SELECT
    ...
    FROM `{PROJECT_ID}.profit_scout.overnight_signals_enriched_v4`
    ...
    for _, row in df.iterrows():
        ...
        # V4: no premium score gate — all enriched signals trade
...
def run_iv_cache_update():
    ...
    `overnight_signals_enriched_v4`, snapshots each underlying's options chain via
