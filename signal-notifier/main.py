...
LIVE_COHORT_START_DATE = "2026-08-21"  # PRINT-FLOOR-25 cohort (reset 2026-08-20, owner call): first ENTRY under PRINT_FLOOR_MIN=25 (raised from 1; slate ghost rate 36.7% -> 9.4% over 31 measured days). Material selection change, so the standing cohort-reset-on-filter-change doctrine applies. Same shape as 06-25 / 07-28 / 08-07 / 08-12: NO truncation — prior rows stay in forward_paper_ledger and are excluded from the public cohort by this date filter; cohort_stats/current recomputes from this constant, so the public panel legitimately shows 0 closed until the first 08-21+ trade closes. policy_version STAYS V7_1_TILTED_GIGO (exit mechanics unchanged; only selection got stricter). This is the FIFTH reset in eight weeks — the 08-12 caution about incremental selection churn stands, and the ghost-scrub is intended as the last selection change before the pool-vs-benchmark test runs. The 2026-08-13 cohort it replaces held 4 entries selected while ghosts could still reach the tournament. See docs/DECISIONS/2026-08-20-score-floor-accepted-print-floor-25-shipped.md and docs/DECISIONS/2026-08-12-failsoft-restore-never-picks.md.
