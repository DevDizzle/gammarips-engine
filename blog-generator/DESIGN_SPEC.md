...
| `read_live_context() -> dict` | writer | Reads closed-trade stats for the live validation cohort (BQ `forward_paper_ledger`, filtered by the live cohort **PAIR** — `policy_version` AND entry on/after `LIVE_COHORT_START_DATE`, from `gammarips_content.cohort`; the label alone is NOT the cohort, since resets since 2026-07-28 are date-filter rather than truncations). Does NOT read `todays_pick` — there is no public pick. Used when the post needs live data (e.g. weekly engine recap). Gated by post `type` — evergreen posts skip. **Fails CLOSED:** any status other than `success` (including a ledger read error) forbids win-rate / P&L claims. |
