# V5.4 Scorer: Options-Flow Candidate Evaluation (v5)
...
- DTE matters: a 7-15 DTE OTM contract has high gamma exposure (good for our hold) but brutal theta (bad if the move stalls). 25+ DTE softens both. Sweet spot: 7-45 DTE. (Band widened 2026-05-12 from the prior 7-30 — the tighter band was over-restricting candidate inventory to a median of 1 candidate/day post-gates, starving the picker. The 31-45 DTE tail is now in-band as guidance, but gamma exposure beyond ~45 DTE goes soft relative to our 3-day hold — see the >45 penalty below.)
...
- `recommended_dte`: Sweet spot 7-45 DTE for our 3-day hold (gamma > theta in the lower half, softer gamma in the 31-45 tail but still acceptable). Anything >45 DTE has soft gamma exposure relative to our 3-day window — penalize.
...
**Calibration Anchors:**
- **10-9:** Exceptional flow + structurally fit. `volume_oi_ratio` > 5, same-direction dollar volume > $1M, `recommended_spread_pct` < 5%, `flow_intent="DIRECTIONAL"`, AND moneyness_pct in [0.05, 0.10] (OTM), DTE in [7, 45] (with the lower half [7, 30] being ideal), mid_price modest enough that +80% is plausible on a 3-4% underlying move.
- **8-7:** Strong flow + adequate structure. `volume_oi_ratio` 2-5, dollar volume ~$500k-$1M, spread < 8%, `flow_intent="DIRECTIONAL"` or `"MIXED"`, moneyness 5-15% OTM, DTE 7-45.
