...
| Time | Action |
|---|---|
| ~9:55 AM ET | Email arrives (or nothing — skip day); expect it ~9:53–9:57 from the 09:52 run (live since the 2026-07-28 liquidity upgrade; was ~9:50). The webapp updates at the same moment (the WhatsApp push is dormant — OpenClaw is decommissioned). The email now carries a `Liquidity: N prints` line — treat CAUTION/UNVERIFIED as a size-down or skip signal. |
...
## The strategy
- Entry: 10:00 AM, via a **limit** at the published entry-day mark (a small buffer makes it marketable) — never market, never chase past the cap
...
## The entry price (2026-06-30, hardened 2026-08-14)
The email + webapp show a **fresh entry-day mark** (a delayed same-day price, date-validated since 2026-08-14 — a prior-session close can never become the mark), a **Limit BUY** price, a **"don't chase above"** cap, and the −30% / +40% dollar levels. Rest your limit there; if the contract runs past the cap before you fill, **skip it** — don't buy the top. A card with a **refusal note and NO bracket levels** is the validation working, not a bug: skip that day. (Reference levels are computed off the mark; your actual fill sets your real bracket.)
...
A day where zero candidates clear the liquidity floors is a correct no-pick day (no restore of sub-floor names since 2026-08-12).
...
The enriched BULLISH slate goes to **signal-judge** (`tournament_v1_3`, `gemini-3.1-pro-preview`). It runs **3 independent randomized brackets**, each: shuffle signals into batches of ≤10 → the LLM picks the top 2 per batch → advance and repeat. Each batch call gets a simple prompt + the daily report + per-contract JSON; **no memory, no rubrics, no weights**. The 3 bracket winners vote: 3/3 agree → `confidence=high`, 2/3 → `medium`, 1/3 → `low`. One ticker emailed. **Fail-closed on any error** — no fallback; signal-judge uptime is the SLO.
...
`overnight-scanner (23:00 ET) → enrichment-trigger (05:30 ET) → overnight-report-generator → signal-notifier (09:52 ET, VERIFIED `52 9 * * 1-5` America/New_York) ← signal-judge (V7.1 tournament_v1_3) → email + Firestore todays_pick → forward-paper-trader (ledger)`
